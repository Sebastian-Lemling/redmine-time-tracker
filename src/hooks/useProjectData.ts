...
export function useProjectData(instanceId?: string) {
  ...
  const fetchProjectMembers = useCallback(
    async (projectId: number) => {
      ...
      try {
        const data = await api<{ members: RedmineMember[] }>(
          `${prefix}/projects/${projectId}/members`,
        );
