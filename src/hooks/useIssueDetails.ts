...
export function useIssueDetails() {
  ...
  const fetchIssueSubject = useCallback(async (issueId: number) => {
    if (fetchingIssueSubjects.current.has(issueId)) return;
    fetchingIssueSubjects.current.add(issueId);
    try {
      const data = await api<{ issue: { id: number; subject: string } }>(`/api/issues/${issueId}`);
  ...
  const fetchIssueDescription = useCallback(async (issueId: number) => {
    ...
    try {
      ...
      }>(`/api/issues/${issueId}?include=journals`);
